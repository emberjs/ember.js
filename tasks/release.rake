namespace :release do
  namespace :framework do
    ...
    task :commit do
      ...
      unless pretend?
        `git reset`
        `git add VERSION CHANGELOG packages/ember-metal/lib/core.js ember.json packages/**/package.json`
        `git commit -m 'Version bump - #{EMBER_VERSION}'`
      end
    end
    ...
    task :tag do
      ...
      `git tag v#{EMBER_VERSION}` unless pretend?
    end
  ...
  namespace :starter_kit do
    ...
    file "tmp/starter-kit" do
      ...
      Dir.chdir("tmp") do
        `git clone https://github.com/emberjs/starter-kit.git`
      end
    end
    ...
    task :update => :index do
      ...
      unless pretend?
        Dir.chdir("tmp/starter-kit") do
          `git add -A`
          `git commit -m 'Updated to #{EMBER_VERSION}'`
          `git tag v#{EMBER_VERSION}`
