...
| ------------------------ | ----------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ember-simple-auth`      | 6.x               | untested     | Pure service + session store, no VM touch expected. Likely `pass` after validation — the domain review's top-10 list rates it "should work". Not exercised in the 14-module spike.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `ember-paper`            | 1.x (legacy)      | classic-only | Legacy addon with many custom component managers and direct `@glimmer/*` reaches. The domain review rates it legacy with many custom managers — assume classic-only. Not targeted for GXT compatibility; recommend consumers migrate to a maintained alternative if they want GXT.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `ember-intl`             | 7.x               | untested     | Helper-manager-based translation helpers + service + runtime locale switching. Helper manager parity is `PASS` in the spike matrix, so the runtime path is plausible, but the addon's direct imports haven't been audited. The domain review rates it "should work once helper manager parity is confirmed".                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `ember-data`             | 5.x / 6.x         | untested     | `ember-data` itself lives in services + computed/tracked state, does not touch the VM directly, and is plausibly `pass` after validation — **but** Ember Data's full test suite was not run against GXT in the spike, and tracked identity stability through relationship caches is the kind of subtle behavior that can drift silently on a reactivity-core swap. The domain review notes `@ember-data/debug` talks to `@ember/debug`, which imports `@glimmer/interfaces` — verify.                                                                                                                                                                                                                                                  |
...
| `tracked-built-ins`      | 3.x               | untested     | Tracked Array/Map/Set/Object wrappers that wrap the `@glimmer/tracking` primitives. Depends on tracked property semantics being identical between backends, which is `PASS` in the spike for the primitive case but `UNVALIDATED` for `(hash)`/`(array)`-style reference-identity behavior. Risk of silent over-invalidation on reference comparisons (the `(hash)`/`(array)` identity concern in the RFC matrix).                                                                                                                                                                                                                                                                                                                     |
...
- **pass**: 7 (`ember-cli-mirage`, `ember-render-modifiers`,
  `ember-modifier`, `ember-fetch`, `ember-cli-app-version`,
  `ember-template-lint`, `ember-truth-helpers`)
...
Tally: **7 pass, 4 classic-only, 9 untested** (total 20).
