...
if (!budget) {
  console.error(`No budget entry for backend '${backend}'. Known: ${Object.keys(budgets).filter((k) => !k.startsWith('_')).join(', ')}`);
  process.exit(1);
}
...
for (const entry of budget.entries) {
  ...
  console.log(`  raw: ${fmt(raw).padStart(10)} / ${fmt(maxRaw).padStart(10)} (${pct(raw, maxRaw)}%)`);
