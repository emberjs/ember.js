import { EMPTY_STRING_ARRAY, extractHandle, isSmallInt, encodeImmediate, IS_COMPILABLE_TEMPLATE, assert as assert$1, unwrap, Stack, isPresentArray, encodeHandle, expect, assign, encodeBoolean, reverse, EMPTY_ARRAY, dict, enumerate, debugToString } from '@glimmer/util';
import { logOpcodeSlice } from '@glimmer/debug';
import { Op, $v0, $fp, TYPE_SIZE, isMachineOp, MACHINE_MASK, ARG_SHIFT, $sp, InternalComponentCapabilities, $s0, $s1, COMPONENT_CONTENT, HELPER_CONTENT, STRING_CONTENT, SAFE_STRING_CONTENT, FRAGMENT_CONTENT, NODE_CONTENT } from '@glimmer/vm';
...
function definePushOp(fn) {
  ...
  op.target = target => labelOperand(target);
  return op;
}
class Compilers {
  #names = {};
...
function debugSymbols(layout) {
  ...
  return DEBUG || hasDebug ? symbols : null;
}
...
function InvokeStaticComponent(op, {
  ...
  let bailOut = symbolTable.hasDebug || hasCapability(capabilities, InternalComponentCapabilities.prepareArgs);
  if (bailOut) {
    InvokeNonStaticComponent(op, {
      capabilities,
      elementBlock,
...
function defineContent(statements) {
