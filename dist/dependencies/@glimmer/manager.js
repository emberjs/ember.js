...
class CustomHelperManager {
  ...
  getHelper(definition) {
    return (capturedArgs, owner) => {
      ...
      if (hasValue(manager)) {
        let cache = Formula(() => manager.getValue(bucket), DEBUG && manager.getDebugName && manager.getDebugName(definition));
        if (hasDestroyable(manager)) {
          associateDestroyableChild(cache, manager.getDestroyable(bucket));
        }
        return cache;
      } else if (hasDestroyable(manager)) {
        let ref = ReadonlyCell(undefined, DEBUG && (manager.getDebugName?.(definition) ?? 'unknown helper'));
