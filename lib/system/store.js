...
var DATA_PROXY = {
  get: function(name) {
    return this.unsavedData[name] || this.savedData[name];
  }
};
...
DS.Store = Ember.Object.extend({
  ...
  */
  ...
  createRecord: function(type, properties, transaction) {
    ...
    DATA_PROXY.savedData = {};
    DATA_PROXY.unsavedData = get(record, 'unsavedData');
  ...
  */
  ...
  hashWasUpdated: function(type, clientId, record) {
    DATA_PROXY.savedData = get(record, 'savedData');
    DATA_PROXY.unsavedData = get(record, 'unsavedData');
  ...
  _didCreateRecord: function(record, hash, data, clientId, primaryKey, idMap, idList) {
    ...
    if (hash) {
      ...
      changes = get(record, 'unsavedData');
      data = get(record, 'savedData');
  ...
  didCreateRecord: function(model, hash) {
    ...
    if (hash) {
      ...
      ember_assert("The server did not return data, and you did not create a primary key (" + primaryKey + ") on the client", get(get(model, 'savedData'), primaryKey));
    }
  ...
  updateModelArrayFilter: function(array, type, filter) {
    ...
    for (var i=0, l=allClientIds.length; i<l; i++) {
      ...
      hash = DATA_PROXY.savedData = data[clientId];

      if (record = recordCache[clientId]) {
        DATA_PROXY.unsavedData = get(record, 'unsavedData');
      } else {
        DATA_PROXY.unsavedData = {};
      }
