...
// Vite/rollup module ids use POSIX separators on every platform, so match
// with `/` regardless of the host OS (and normalize just in case a resolver
// hands us a Windows-style path).
...
export function deprecationShaking({ compliantThrough, strip = [], keep = [] } = {}) {
  ...
  return {
    ...
    load(id) {
      ...
      if (path.endsWith(FLAGS_MODULE_SUFFIX) && path.includes('/dist/')) {
        return code;
      }
    },
  };
