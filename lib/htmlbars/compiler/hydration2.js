...
prototype.helper = function(name, size, escaped, parentPath, startIndex, endIndex) {
  ...
  this.pushMustache("["+string(name)+", "+prepared.args+", "+hash(prepared.options)+"]", parentPath, startIndex, endIndex);
};
...
prototype.pushMustache = function(mustache, parentPath, startIndex, endIndex) {
  var parent = "fragment";
  ...
  this.mustaches.push("{range:"+range+",mustache:"+mustache+"}");
};
