...
SC.mixin(SC.Object, /** @scope SC.Object */ {
  ...
  */
  extend: function(props) {
    ...
    // now we can walk a class hierarchy
    ret.subclasses = SC.Set.create();
    this.subclasses.add(ret);
...
SC.Object.prototype = /** @scope SC.Object.prototype */{
  ...
  */
  _object_init: function(extensions) {
    ...
    for (idx = 0; idx < len; idx++) {
      SC._object_extend(this, extensions[idx], this.__sc_super__);
    }
...
SC._object_className = function(obj) {
  // class names are not available until ready
  if (SC.isReady === NO) { return ''; }
  ...
  // match.
  var ret = obj;
  while (ret && !ret._object_className) {
    ret = ret.superclass;
  }
