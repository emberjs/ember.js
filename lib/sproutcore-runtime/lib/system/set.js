...
*/
SC.Set = SC.mixin({}, SC.Enumerable, SC.Observable, SC.Freezable, SC.Copyable,
/** @scope SC.Set.prototype */ {
  ...
  */
  create: function(items) {
    ...
    if (!isObservable && items === undefined && pool.length > 0) { return pool.pop(); }
  ...
  */
  isEqual: function(obj) {
    // fail fast
    if (!obj || !obj.isSet || (obj.get('length') !== this.get('length'))) { return NO; }
  ...
  */
  addEach: function(objects) {
    ...
    if (!objects || !objects.isEnumerable) {
      throw new SC.Error("%@.addEach must pass enumerable".fmt(this));
    }
  ...
  */
  removeEach: function(objects) {
    ...
    if (objects.isSCArray) {
      idx = objects.get('length');
      while(--idx >= 0) {
        this.remove(objects.objectAt(idx));
      }
    } else if (objects.isSet) {
      idx = objects.length;
      while(--idx>=0) {
        this.remove(objects[idx]);
      }
    } else {
      objects.forEach(function(i) {
        this.remove(i);
  ...
  */
  ...
  forEach: function(iterator, target) {
    ...
    for (var idx = 0; idx < len; idx++) {
      iterator.call(target, this[idx], idx, this);
    }
    return this;
  },
