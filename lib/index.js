...
const shim = addonV1Shim(path.join(__dirname, '..'), {
  autoImportCompat: {
    customizeMeta(meta) {
      ...
      // ember-auto-import does not have global visibility of all imports in all
      // v1 addons, so we inject implicit-modules to force all
      // traditionally-included modules to be included.
      //
...
module.exports = {
  ...
  treeForVendor() {
    ...
    let emberVendorShims = new MergeTrees([
    ...
    return emberCliBabel.transpileTree(emberVendorShims, {
      'ember-cli-babel': {
