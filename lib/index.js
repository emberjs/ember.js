...
module.exports = {
  ...
  treeForAddon(tree) {
    ...
    let result = this.preprocessJs(this.buildPackages(tree, isProduction), '/', this.name, {
  ...
  buildPackages(distTree, shouldCompileModules) {
    const isProduction = process.env.EMBER_ENV === 'production';
    if (this._overrideTree) {
      distTree = new MergeTrees([distTree, this._overrideTree], { overwrite: true });
    }
    let packages = this.transpileTree(
      new Funnel(distTree, { srcDir: 'packages' }),
      isProduction,
      shouldCompileModules
    );

    let dependencies = this.transpileTree(
      new Funnel(distTree, { srcDir: 'dependencies' }),
      isProduction,
      shouldCompileModules
    );
    return debugTree(new MergeTrees([packages, dependencies], 'buildPackages'));
  },

  buildEmberBundles(tree, isProduction) {
    let packages = this.buildPackages(tree, isProduction, tree);
    ...
    let emberFiles = new MergeTrees([new Funnel(packages, { exclude }), headerFiles]);
    ...
    emberFiles = debugTree(emberFiles, 'emberFiles');
    emberTestingFiles = debugTree(emberTestingFiles, 'emberTestingFiles');
