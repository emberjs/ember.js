...
module.exports = {
  init() {
    ...
    // When using strict-es-modules, the packages are our treeForAddon
    this.treePaths.addon = 'dist/packages';
  },
  ...
  treeForAddon(tree) {
    let optionalFeatures = this.project.addons.find((a) => a.name === '@ember/optional-features');
    if (optionalFeatures.isFeatureEnabled('strict-es-modules')) {
  ...
  treeForVendor(tree) {
    let optionalFeatures = this.project.addons.find((a) => a.name === '@ember/optional-features');
    if (optionalFeatures.isFeatureEnabled('strict-es-modules')) {
      // rely on actual ES modules in treeForAddon instead
      return;
    }
