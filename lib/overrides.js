...
module.exports = class Overrides {
  static for(project, env = process.env) {
    const isProduction = env.EMBER_ENV === 'production';
    const globalDeprecations = env.EMBER_GLOBAL_DEPRECATIONS === 'all';
    if (isProduction) {
      return new Overrides([]);
    }
    return new Overrides(Overrides.addonsInfoFor(project), {
        showAllEmberGlobalDeprecations: globalDeprecations,
    });
  }
