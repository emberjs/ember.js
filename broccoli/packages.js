...
const transpileESProposals = require('./transforms/transpile-es-proposals');
...
module.exports.getPackagesES = function getPackagesES() {
  ...
  let transpiledNonTSContents = debugTree(
    transpileESProposals(debugTree(nonTypeScriptContents, `get-packages-es:babel-plugins:input`)),
  ...
  let mergedFinalOutput = new MergeTrees([transpiledNonTSContents, debuggedCompiledTypescript], {
    overwrite: true,
  });
