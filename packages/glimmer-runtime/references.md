...
## More Efficient Composition Via Notification

When doing a lot of composition using the base type, each intermediate link has to call `isDirty()` on all of its inputs, which can quickly add up.
