  ...
  if (!helper) {
    ...
    if (validateLazyHelperName(name, container, env.hooks.keywords, env.knownHelpers)) {
      ...
      if (container._registry.has(helperName)) {
        ...
        Ember.assert(`Expected to find an Ember.Helper with the name ${helperName}, but found an obejct of type ${typeof helper} instead.`, helper.isHelperFactory || helper.isHelperInstance);
      }
    }
  }
