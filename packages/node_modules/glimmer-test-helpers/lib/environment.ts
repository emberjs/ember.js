...
class EmberishCurlyComponentManager implements ComponentManager<EmberishCurlyComponent> {
  create(definition: EmberishCurlyComponentDefinition, args: EvaluatedNamedArgs): EmberishCurlyComponent {
    let klass = definition.ComponentClass || EmberishCurlyComponent;
    let attrs = args.value();
    let component = klass.create({ attrs });
...
export class TestEnvironment extends Environment {
  ...
  registerEmberishCurlyComponent(name: string, Component: ComponentClass, layout: string): any {
    // let testHooks = new HookIntrospection(hooks);
    let definition = new EmberishCurlyComponentDefinition(EMBERISH_CURLY_COMPONENT_MANAGER, Component, this.compileLayout(layout));
    return this.registerComponent(name, definition);
  }
...
interface EmberishCurlyGlimmerComponentFactory {
...
class EmberishCurlyComponentDefinition extends GenericComponentDefinition<EmberishCurlyComponent> {
  public ComponentClass: EmberishCurlyGlimmerComponentFactory;
