...
export class TestEnvironment extends Environment {
  ...
  getRootDynamicScope(): TestKeywords {
    return new TestKeywords(null);
  }
...
class TestKeywords implements DynamicScope {
  view: Opaque;

  constructor(view: Opaque) {
    this.view = view;
  }

  child(): TestKeywords {
    return new TestKeywords(this.view);
  }
}
...
class WithKeywordsSyntax extends StatementSyntax {
  ...
  compile(compiler: StatementCompilationBuffer, env: Environment) {
    let map = this.args.named.compile(compiler, env).map;

    let callback = (vm: VM, keywords: TestKeywords) => {
      keywords.view = map['view'].evaluate(vm as any);
    };
