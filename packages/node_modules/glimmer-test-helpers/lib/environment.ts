...
interface IDefinitionLookup {
  definition: BasicComponentDefinition | EmberishCurlyComponentDefinition | EmberishGlimmerComponentDefinition;
  layout: string;
  factory: BasicComponentFactory | EmberishCurlyComponentFactory | EmberishGlimmerComponentFactory,
  manager: BasicComponentManager | EmberishCurlyComponentManager | EmberishGlimmerComponentManager
}

export class TestEnvironment extends Environment {
  private helpers = {};
  private components = dict<ComponentDefinition<any>>();
  private componentDefinitions = dict<IDefinitionLookup>();
  ...
  private registerComponentDefinition(name, Definition, manager, Factory, layout) : IDefinitionLookup {
    if (!this.componentDefinitions[name]) {
      return this.componentDefinitions[name] = {
  ...
  registerBasicComponent<T extends BasicComponent>(name: string, Component: BasicComponentFactory, layout: string): IDefinitionLookup {
    return this.registerComponentDefinition(name, BasicComponentDefinition, BASIC_COMPONENT_MANAGER, Component, layout);
  }

  registerEmberishCurlyComponent(name: string, Component: ComponentClass, layout: string): IDefinitionLookup {
    return this.registerComponentDefinition(name, EmberishCurlyComponentDefinition, EMBERISH_CURLY_COMPONENT_MANAGER, Component, layout);
  }

  registerEmberishGlimmerComponent(name: string, Component: EmberishGlimmerComponentFactory, layout: string): IDefinitionLookup {
    return this.registerComponentDefinition(name, EmberishGlimmerComponentDefinition, EMBERISH_GLIMMER_COMPONENT_MANAGER, Component, layout);
  }
  ...
  getComponentDefinition(name: InternedString[], syntax: StatementSyntax): ComponentDefinition<any> {
    let componentName = <string>name[0];
    let definition = this.components[componentName];
    let componentLookup = this.componentDefinitions[componentName];
    ...
    if (componentLookup) {
      let Definition : any = componentLookup.definition;
      let { factory: Factory, manager, layout } = componentLookup;
      let componentDefinition : ComponentDefinition<any> = new Definition(name, manager, Factory, this.compileLayout(layout));
...
abstract class GenericComponentDefinition<T extends Component> extends ComponentDefinition<T> {
  public compiledLayout : CompiledLayout;
...
class EmberishCurlyComponentDefinition extends GenericComponentDefinition<EmberishCurlyComponent> {
  ...
  constructor(name: string, manager: ComponentManager<any>, ComponentClass: ComponentClass, layout) {
    super(name, manager, ComponentClass, layout);
    this.layout = layout;
  }
