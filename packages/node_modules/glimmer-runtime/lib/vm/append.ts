...
import { Dict, FIXME, Stack, LinkedList, LOGGER, InternedString, dict, assert } from 'glimmer-util';
...
export default class VM {
  ...
  static initial(env: Environment, { elementStack, self, size }: VMOptions) {
    let scope = env.createRootScope(self, size);
  ...
  pushRootScope(self: PathReference, size: number): Scope {
    let scope = this.env.createRootScope(self, size);
    this.scopeStack.push(scope);
    return scope;
  }
  ...
  execute(opcodes: OpSeq, initialize?: (vm: VM) => void): RenderResult {
    ...
    return new RenderResult({
      ...
      self: self as FIXME // PathReference -> UpdatableReference
    });
  }
