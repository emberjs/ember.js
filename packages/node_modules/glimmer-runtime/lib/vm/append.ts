...
import { ListBlockOpcode, TryOpcode, BlockOpcode } from './update';
...
export default class VM {
  ...
  static initial(env: Environment, { elementStack, self, size }: VMInitialOptions) {
    let scope = Scope.root(self, size);
    let dynamicScope = DynamicScope.root(env.getKeywords().length);
  ...
  bindKeywords(entries: [number, CompiledExpression][]) {
    ...
    entries.forEach(([symbol, expr]) => {
      scope.bindSymbol(symbol, expr.evaluate(this));
    });
  }
}
