...
import { ListSlice, Slice, Dict, dict, assign, installGuid } from 'glimmer-util';
...
export class BindNamedArgsOpcode extends Opcode {
  ...
  toJSON(): OpcodeJSON {
    return {
      guid: this._guid,
      type: this.type,
      args: [`[${Object.keys(this.named).map(name => JSON.stringify(name)).join(", ")}]`]
    };
  }
}
...
export class EnterOpcode extends Opcode {
  ...
  toJSON(): OpcodeJSON {
    ...
    let begin = this.slice.head() as LabelOpcode;
    let end = this.slice.tail() as LabelOpcode;
