...
export class OpenComponentOpcode extends Opcode {
  ...
  evaluate(vm: VM) {
    ...
    vm.updateWith(new UpdateComponentOpcode({ component, manager, args }));
  }
...
export class UpdateComponentOpcode extends UpdatingOpcode {
  ...
  constructor({ component, manager, args } : { component: Component, manager: ComponentManager<any>, args: EvaluatedArgs }) {
    super();
  ...
  toJSON(): OpcodeJSON {
    let guid = this.component["_guid"];

    if (guid) {
      return {
        type: this.type,
        args: [JSON.stringify(`ember${guid}`)]
      };
    } else {
      return super.toJSON();
    }
