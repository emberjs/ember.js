...
import { FIXME, HasGuid, assign } from "glimmer-util";
...
class InvokeAs {
  ...
  static create(invokeAs: InvokeAsOptions, block: string) {
    if (typeof block === 'string') {
      invokeAs = invokeAs || {};

      if ("template" in invokeAs) {
        throw new Error("Cannot provide both a block and invokeAs.template");
      }

      invokeAs.template = block;
    }
    ...
    return new InvokeAs(invokeAs);
  }

  constructor({ attrs, context, blockParams, template, inverse }: InvokeAsOptions) {
    this.attrs = attrs;
...
class CurlySyntaxGenerator {
  invokeAs: InvokeAs;

  constructor(invokeAs: InvokeAs) {
    this.invokeAs = invokeAs;
  }

  generate(): string {
    let { attrs: rawAttrs, context, blockParams, template, inverse } = this.invokeAs;

    let attrs = this.attrList(rawAttrs);

    if (typeof template === 'string') {
      let args = blockParams ? ` as |${blockParams.join(' ')}|` : '';
      let inv = typeof inverse === 'string' ? `{{else}}${inverse}` : '';
      return `{{#test-component ${attrs}${args}}}${template}${inv}{{/test-component}}`;
    } else {
      return `{{test-component ${attrs}}}`;
    }
  }

  protected attrList(attrs: Object): string {
    let attrList = Object.keys(attrs).map((name) => {
      return `${name}=${this.attrValue(attrs[name])}`;
    });

    return attrList.join(' ');
  }

  protected attrValue(value: any): string {
    switch (typeof value) {
      case 'object':
...
function testComponent(title: string, { kind, layout, invokeAs, expected: _expected, skip }: ComponentTestOptions) {
  if (skip === true) return;

  let { attrs = {}, props = {}, context, blockParams, template, inverse } = (invokeAs || <InvokeAsOptions>{});
  ...
  if (!kind || kind === 'curly') {
    ...
    test(`curly: ${title}`, assert => {
      ...
      assert.ok(true, `generated syntax: ${syntax}`);
      appendViewFor(syntax, context || {});
  ...
  if (!kind || kind === 'glimmer') {
    ...
    test(`glimmer: ${title}`, assert => {
      env.registerEmberishGlimmerComponent('test-component', EmberishGlimmerComponent, ` <aside>${layout}</aside><!-- hi -->`);
      ...
      if (typeof template === 'string') {
        syntax = `<${tag}>${template}</test-component>`;
      } else {
        syntax =`<${tag} />`;
      }
...
testComponent('shadowing: non-block with properties on attrs', {
  kind: 'glimmer',
  layout: 'In layout - someProp: {{@someProp}}',
