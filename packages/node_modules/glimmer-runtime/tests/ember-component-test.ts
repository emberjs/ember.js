...
import { FIXME, assign } from "glimmer-util";
...
function testComponent(title: string, { kind, layout, invokeAs, block, expected: _expected, skip }: ComponentTestOptions) {
  ...
  if (!kind || kind === 'glimmer') {
    QUnit.test(`basic: ${title}`, () => {
      env.registerBasicComponent('test-component', BasicComponent, ` <aside>${layout}</aside><!-- hi -->`);
  ...
  //   env.registerBasicComponent('test-component', GlimmerComponent, layout);
  //   env.registerBasicComponent('test-component2', GlimmerComponent, layout2);
...
testComponent('correct scope - conflicting block param and attr names', {
  ...
  invokeAs: { attrs: { item: '"from block"', conflict: '"from attr"' } },
...
QUnit.test('correct scope - simple', assert => {
  env.registerBasicComponent('sub-item', EmberishComponent,
    `<p>{{@name}}</p>`
  );
...
QUnit.test('correct scope - complex', assert => {
  ...
  env.registerBasicComponent('my-item', EmberishComponent,
    stripTight`
      <aside>{{@item.id}}:
        {{#if @item.visible}}
...
QUnit.skip('rerendering component with attrs from parent', function() {
  ...
  let hooks = </* HookIntrospection */ FIXME>env.registerEmberishComponent('non-block', NonBlock, 'In layout - someProp: {{someProp}}').hooks;
