import { Template, RenderResult, DynamicScope, VM } from "glimmer-runtime";
...
function render(template: Template, context={}, suppliedOptions={}) {
  let options = assign({ appendTo: root }, suppliedOptions);
...
test("dynamically scoped keywords can be passed to render, and used in curlies", assert => {
  ...
  function dynamicScope(vm: VM, keywords: DynamicScope) {
    keywords['view'] = viewRef;
  }

  render(template, {}, { dynamicScope });
...
test("changing dynamic scope", assert => {
  ...
  render(template, { innerView }, { dynamicScope });
