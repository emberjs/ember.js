...
moduleFor('Location Utilities', class extends AbstractTestCase {
  ...
  ['@test Feature-Detecting onhashchange'](assert) {
    assert.equal(supportsHashChange(undefined, { onhashchange() {} }), true, 'When not in IE, use onhashchange existence as evidence of the feature');
    ...
    assert.equal(supportsHashChange(7, { onhashchange() {} }), false, 'When in IE7 compatibility mode, never report existence of the feature');
    assert.equal(supportsHashChange(8, { onhashchange() {} }), true, 'When in IE8+, use onhashchange existence as evidence of the feature');
  }

  ['@test Feature-detecting the history API'](assert) {
    assert.equal(supportsHistory("", { pushState: true }), true, "returns true if not Android Gingerbread and history.pushState exists");
