...
interface LogCallbackObject {
    /**
    * The boolean result of an assertion, true means passed, false means
    * failed.
    */
    result: boolean;
    ...
    */
    ...
    /**
    * The associated stacktrace, either from an exception or pointing to the
    * source of the assertion. Depends on browser support for providing
    * stacktraces, so can be undefined.
    */
    source: string;
}
...
interface QUnitAssert {
    ...
    *
    * When your test has any asynchronous exit points, call assert.async() to
    * get a unique resolution callback for each async operation. The callback
    * returned from assert.async() will throw an Error if is invoked more than
    * once.
    */
    async(): () => void;
    ...
    * the value of objects, such that { key: value } is equal to { key: value
    ...
    */
    ...
    /**
    * A non-strict comparison assertion, roughly equivalent to JUnit
    * assertEquals.
    *
    * The equal assertion uses the simple comparison operator (==) to compare
    * the actual and expected arguments. When they are equal, the assertion
    * passes: any; otherwise, it fails.  When it fails, both actual and
    ...
    * use expect( number ) to register an expected count. If the number of
    * assertions run does not match the expected count, the test will fail.
    *
    ...
    */
    ...
    /**
    * An inverted deep recursive comparison assertion, working on primitive
    * types, arrays, objects, regular expressions, dates and functions.
    *
    * The notDeepEqual() assertion can be used just like equal() when comparing
    * the value of objects, such that { key: value } is equal to { key: value
    * }. For non-scalar values, identity will be disregarded by notDeepEqual.
    *
    * @param actual Object or Expression being tested
    * @param expected Known comparison value
    * @param message A short description of the assertion
    ...
    * assertion passes: any; otherwise, it fails. When it fails, both actual
    * and expected values are displayed in the test result, in addition to a
    * given message.
    *
    * @param actual Expression being tested
    * @param expected Known comparison value
    ...
    * equal, the assertion passes: any; otherwise, it fails. When it fails,
    ...
    */
    ...
    /**
    * A boolean assertion, equivalent to CommonJS’s assert.ok() and JUnit’s
    ...
    * specific set of circumstances, use throws() to catch the error object for
    ...
    * called raises. It has the same signature and behaviour, just a different
    * name.
    *
    * @param block Function to execute
    * @param expected Error Object to compare
...
interface QUnitStatic extends QUnitAssert {
    ...
    *
    * When your async test has multiple exit points, call start() for the
    * corresponding number of stop() increments.
    *
    * @param decrement Optional argument to merge multiple start() calls into
    * one. Use with multiple corrsponding stop() calls.
    */
    start(decrement?: number): any;

    /**
    * Stop the testrunner to wait for async tests to run. Call start() to
    * continue.
    *
    * When your async test has multiple exit points, call stop() with the
    * increment argument, corresponding to the number of start() calls you
    * need.
    *
    * On Blackberry 5.0, window.stop is a native read-only function. If you
    * deal with that browser, use QUnit.stop() instead, which will work
    * anywhere.
    *
    * @param decrement Optional argument to merge multiple stop() calls into
    * one. Use with multiple corrsponding start() calls.
    */
    ...
    *
    * QUnit.begin() is called once before running any tests. (a better would've
    * been QUnit.start, but thats already in use elsewhere and can't be
    * changed.)
    *
    ...
    * Add an asynchronous test to run. The test must include a call to start().
    *
    * For testing asynchronous code, asyncTest will automatically stop the test
    * runner and wait for your code to call start() to continue.
    *
    * @param name Title of unit being tested
    * @param test Function to close over assertions
    */
    ...
    *
    * All tests that occur after a call to module() will be grouped into that
    * module.  The test names will all be preceded by the module name in the
    * test results.  You can then use that module name to select tests to run.
    *
    ...
    * methods.  If you are avoiding using any of QUnit's globals, you can use
    * the assert argument instead.
    *
    ...
    */
    ...
    skip(title: string, expected: number, test: (assert: QUnitAssert) => any): any;
    ...
    */
    ...
    skip(title: string, test: (assert: QUnitAssert) => any): any;
...
/**
* A deep recursive comparison assertion, working on primitive types, arrays,
* objects, regular expressions, dates and functions.
*
* The deepEqual() assertion can be used just like equal() when comparing the
* value of objects, such that { key: value } is equal to { key: value }. For
* non-scalar values, identity will be disregarded by deepEqual.
*
* @param actual Object or Expression being tested
* @param expected Known comparison value
* @param message A short description of the assertion
...
* any; otherwise, it fails.  When it fails, both actual and expected values are
...
* value of objects, such that { key: value } is equal to { key: value }. For
* non-scalar values, identity will be disregarded by notDeepEqual.
*
* @param actual Object or Expression being tested
* @param expected Known comparison value
* @param message A short description of the assertion
...
* A non-strict comparison assertion, checking for inequality.
*
* The notEqual assertion uses the simple inverted comparison operator (!=) to
* compare the actual and expected arguments. When they aren't equal, the
...
*
* The notStrictEqual assertion uses the strict inverted comparison operator
* (!==) to compare the actual and expected arguments. When they aren't equal,
* the assertion passes; otherwise, it fails. When it fails, both actual and
* expected values are displayed in the test result, in addition to a given
* message.
*
* @param actual Expression being tested
...
* Assertion to test if a callback throws an exception when run.
*
* When testing code that is expected to throw an exception based on a specific
* set of circumstances, use throws() to catch the error object for testing and
* comparison.
*
* @param block Function to execute
* @param expected Error Object to compare
...
* corresponding number of stop() increments.
*
* @param decrement Optional argument to merge multiple start() calls into one.
* Use with multiple corrsponding stop() calls.
*/
...
*
* When your async test has multiple exit points, call stop() with the increment
* argument, corresponding to the number of start() calls you need.
*
* On Blackberry 5.0, window.stop is a native read-only function. If you deal
* with that browser, use QUnit.stop() instead, which will work anywhere.
*
* @param decrement Optional argument to merge multiple stop() calls into one.
* Use with multiple corrsponding start() calls.
*/
...
*
* QUnit.begin() is called once before running any tests. (a better would've
* been QUnit.start, but thats already in use elsewhere and can't be changed.)
*
* @param callback Callback to execute
*/
...
* Register a callback to fire whenever an assertion completes.
*
* This is one of several callbacks QUnit provides. Its intended for integration
* scenarios like PhantomJS or Jenkins. The properties of the details argument
* are listed below as options.
*
* @param callback Callback to execute.
*/
...
//declare let module: (name: string, lifecycle?: LifecycleObject) => any;

/**
* Add a test to run.
*
* When testing the most common, synchronous code, use test().  The assert
* argument to the callback contains all of QUnit's assertion methods.  If you
* are avoiding using any of QUnit's globals, you can use the assert argument
* instead.
*
* @param title Title of unit being tested
* @param expected Number of assertions in this test
