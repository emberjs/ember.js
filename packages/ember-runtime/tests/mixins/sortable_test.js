...
QUnit.test('if you do not specify `sortProperties` sortable have no effect', function() {
  equal(sortedArrayProxy.get('length'), 3, 'array has 3 items');
  equal(sortedArrayProxy.objectAt(0).name, 'Scumbag Dale', 'array is in it natural order');

  unsortedArray.pushObject({ id: 4, name: 'Scumbag Chavard' });

  equal(sortedArrayProxy.get('length'), 4, 'array has 4 items');
  equal(sortedArrayProxy.objectAt(3).name, 'Scumbag Chavard', 'a new object was inserted in the natural order');

  sortedArrayProxy.set('sortProperties', []);
  unsortedArray.pushObject({ id: 5, name: 'Scumbag Jackson' });

  equal(sortedArrayProxy.get('length'), 5, 'array has 5 items');
  equal(sortedArrayProxy.objectAt(4).name, 'Scumbag Jackson', 'a new object was inserted in the natural order with empty array as sortProperties');
});

QUnit.test('you can change sorted properties', function() {
  sortedArrayProxy.set('sortProperties', ['id']);

  equal(sortedArrayProxy.objectAt(0).name, 'Scumbag Dale', 'array is sorted by id');
  equal(sortedArrayProxy.get('length'), 3, 'array has 3 items');

  sortedArrayProxy.set('sortAscending', false);

  equal(sortedArrayProxy.objectAt(0).name, 'Scumbag Bryn', 'array is sorted by id in DESC order');
  equal(sortedArrayProxy.objectAt(2).name, 'Scumbag Dale', 'array is sorted by id in DESC order');
  equal(sortedArrayProxy.get('length'), 3, 'array has 3 items');

  sortedArrayProxy.set('sortProperties', ['name']);

  equal(sortedArrayProxy.objectAt(0).name, 'Scumbag Katz', 'array is sorted by name in DESC order');
  equal(sortedArrayProxy.get('length'), 3, 'array has 3 items');
});
...
QUnit.test('changing sortProperties and sortAscending with setProperties, sortProperties appearing first', function() {
  ...
  equal(sortedArrayProxy.objectAt(0).name, 'Scumbag Katz', 'array is sorted by name in DESC order');
  equal(sortedArrayProxy.objectAt(2).name, 'Scumbag Bryn', 'array is sorted by name in DESC order');

  sortedArrayProxy.setProperties({ sortProperties: ['id'], sortAscending: true });

  equal(sortedArrayProxy.objectAt(0).id, 1, 'array is sorted by id in ASC order after setting sortAscending and sortProperties');
  equal(sortedArrayProxy.objectAt(2).id, 3, 'array is sorted by id in ASC order after setting sortAscending and sortProperties');

  sortedArrayProxy.setProperties({ sortProperties: ['name'], sortAscending: false });

  equal(sortedArrayProxy.objectAt(0).name, 'Scumbag Katz', 'array is sorted by name in DESC order after setting sortAscending and sortProperties');
  equal(sortedArrayProxy.objectAt(2).name, 'Scumbag Bryn', 'array is sorted by name in DESC order after setting sortAscending and sortProperties');

  sortedArrayProxy.setProperties({ sortProperties: ['id'], sortAscending: false });

  equal(sortedArrayProxy.objectAt(0).id, 3, 'array is sorted by id in DESC order after setting sortAscending and sortProperties');
  equal(sortedArrayProxy.objectAt(2).id, 1, 'array is sorted by id in DESC order after setting sortAscending and sortProperties');
...
QUnit.test('sortable object will expose associated content in the right order', function() {
  equal(sortedArrayProxy.get('length'), 3, 'array has 3 items');
  equal(sortedArrayProxy.objectAt(0).name, 'Scumbag Bryn', 'array is sorted by name');
});
...
QUnit.test('you can add objects in sorted order', function() {
  ...
  equal(sortedArrayProxy.get('length'), 4, 'array has 4 items');
  equal(sortedArrayProxy.objectAt(1).name, 'Scumbag Chavard', 'a new object added to content was inserted according to given constraint');

  sortedArrayProxy.addObject({ id: 5, name: 'Scumbag Fucs' });

  equal(sortedArrayProxy.get('length'), 5, 'array has 5 items');
  equal(sortedArrayProxy.objectAt(3).name, 'Scumbag Fucs', 'a new object added to controller was inserted according to given constraint');
});
...
QUnit.test('you can change a sort property and the content will rearrange', function() {
  equal(sortedArrayProxy.get('length'), 3, 'array has 3 items');
  equal(sortedArrayProxy.objectAt(0).name, 'Scumbag Bryn', 'bryn is first');

  set(sortedArrayProxy.objectAt(0), 'name', 'Scumbag Fucs');
  equal(sortedArrayProxy.objectAt(0).name, 'Scumbag Dale', 'dale is first now');
  equal(sortedArrayProxy.objectAt(1).name, 'Scumbag Fucs', 'foucs is second');
});

QUnit.test('you can change the position of the middle item', function() {
  equal(sortedArrayProxy.get('length'), 3, 'array has 3 items');

  equal(sortedArrayProxy.objectAt(1).name, 'Scumbag Dale', 'Dale is second');
  set(sortedArrayProxy.objectAt(1), 'name', 'Alice'); // Change Dale to Alice

  equal(sortedArrayProxy.objectAt(0).name, 'Alice', 'Alice (previously Dale) is first now');
});
...
QUnit.test('you can set content later and it will be sorted', function() {
  equal(sortedArrayProxy.get('length'), 0, 'array has 0 items');
...
QUnit.test('you can sort with custom sorting function', function() {
  ...
  equal(sortedArrayProxy.get('length'), 3, 'array has 3 items');
  equal(sortedArrayProxy.objectAt(0).name, 'Scumbag bryn', 'array is sorted by custom sort');
});
