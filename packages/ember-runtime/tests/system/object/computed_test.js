...
testWithDefault('replacing computed property with regular val', function(
  get,
  set,
  assert
) {
...
testWithDefault(
  'complex dependent keys changing complex dependent keys',
...
QUnit.test('can retrieve metadata for a computed property', function(assert) {
  ...
  assert.equal(
    emberGet(MyClass.metaForProperty('computedProperty'), 'key'),
    'keyValue',
...
QUnit.test(
  'overriding a computed property with null removes it from eachComputedProperty iteration',
  function(assert) {
    let MyClass = EmberObject.extend({
      foo: computed(function() {}),
...
QUnit.test(
  'can iterate over a list of computed properties for a class',
  function(assert) {
    let MyClass = EmberObject.extend({
      foo: computed(function() {}),
...
QUnit.test(
  'list of properties updates when an additional property is added (such cache busting)',
  function(assert) {
    let MyClass = EmberObject.extend({
      foo: computed(K),
...
QUnit.test(
  ...
  function(assert) {
    ...
    assert.ok(
      emberGet(SubClass.create(), 'foo'),
      'FOO',
      'super value is fetched'
    );
  }
);
...
QUnit.test(
  'observing computed.reads prop and overriding it in create() works',
  function(assert) {
