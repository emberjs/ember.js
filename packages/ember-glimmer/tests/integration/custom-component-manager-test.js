...
if (GLIMMER_CUSTOM_COMPONENT_MANAGER) {
  moduleFor('Components test: curly components with custom manager', class extends RenderingTest {
    ...
    ['@test it can customize the template context']() {
      ...
      this.registerCustomComponentManager({
        getContext() { return customContext; }
      });
    ...
    ['@test arguments are updated if they change']() {
      ...
      this.runTask(() => setProperties(this.context, {
        firstName: "Chad", lastName: "Hietala"
      }));
    ...
    [`@test custom components appear in parent view's childViews array`](assert) {
      ...
      assert.equal(childViews.length, 1, 'turbo component is removed from parent\'s child views array');
    ...
    ['@test can invoke classic components in custom components'](assert) {
      ...
      assert.strictEqual(customComponent.childViews, undefined, 'custom component does not have childViews property');

      childViews = getChildViews(customComponent);
      assert.equal(childViews.length, 1, 'custom component has one child view');
      assert.ok(childViews[0].isClassicComponent, 'custom component child view is classic component');

      assert.ok(classicComponent.parentView.isCustomComponent, `classic component's parentView is custom component`);
    }
  });
}
