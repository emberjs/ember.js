...
function managementForQp(routeInfos: InternalRouteInfo[], qp: QueryParam) {
  ...
  assert(
    ...
    manager && hasClassicInterop(manager) && bucket instanceof ClassicRouteBucket
  );
...
export function finalizeQueryParamChange(
  ...
  for (let qp of qpMeta.qps) {
    let { manager: qpManager, bucket: qpBucket } = managementForQp(routeInfos, qp);
