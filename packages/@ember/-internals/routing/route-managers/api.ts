/**
  Route Manager API. Defines the contract between the router and route base
  classes.

  This module exposes type-only declarations and the `routeCapabilities`
  factory.
*/

import type { Capabilities, Destroyable } from '@glimmer/interfaces';
import type Owner from '@ember/owner';
import type { InternalRouteInfo, Route, RouteInfo, Transition } from 'router_js';
...
export interface RouteManager<Bucket extends RouteStateBucket = RouteStateBucket> {
   ...
   */
  getDestroyable(bucket: Bucket): Destroyable | null;
...
> extends RouteManager<Bucket> {
  ...
  /**
    ...
    route hierarchy.
   */
  qp(bucket: Bucket): QueryParamMeta;
