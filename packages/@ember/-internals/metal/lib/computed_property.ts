...
import { combine, consumeTag, Tag, track, untrack, UpdatableTag, updateTag, validateTag, valueForTag } from '@glimmer/validator';
...
import { ComputedDescriptor, DecoratorPropertyDescriptor } from './decorator';
import { isClassicDecorator } from './descriptor_map';
...
export const DEEP_EACH_REGEX = /\.@each\.[^.]+\./;
  ...
  */
  get(obj: object, keyName: string): any {
    ...
    if (cache.has(keyName) && validateTag(propertyTag, getLastRevisionFor(obj, keyName))) {
      ...
      assert(`Attempted to access the computed ${obj}.${keyName} on a destroyed object, which is not allowed`, this._dependentKeys === undefined || !metaFor(obj).isMetaDestroyed());
