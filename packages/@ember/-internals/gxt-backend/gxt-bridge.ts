...
export interface GxtCompilePipelineCapabilities {
   ...
   * the module lifetime (slice-69/90 stable-collection-identity precedent);
   * contents are mutated in place via `pushHelperInstance` (writers) and
   ...
   *      between tests.
   *
   * Pre-slice-116 reader pattern (2 sites, cross-file):
   ...
   *
   * Slice-116 (Cluster B): graduates the canonical state from the pre-
   * slice-116 globalThis lazy-init at compile.ts:7878
   * (`(globalThis as any).__gxtHelperInstances = (globalThis as any).
   * __gxtHelperInstances || [];`) to the module-local `_gxtHelperInstances`
   * array in `compile.ts` (at the same source position). All cross-file
   * readers route through `getGxtRenderer()?.compilePipeline.
   * getHelperInstances?.()`. Bridge shape mirrors slice-69's
   * `getWrapperUserFalseSet` (get-only collection-return accessor) and
   * slice-90's `getEngineInstances` (stable-collection-identity, lazy-init
   * eliminated by always-initialized module-local declaration). Net -1
   * globalThis slot.
   *
   ...
   * guards short-circuit — matches pre-slice-116 semantics where the slot
   * was undefined → the destroy loop skipped silently.
   *
   * Fast-check: 1 closure read (returning the captured `_gxtHelperInstances`
   * reference); zero allocations on every call. Identical hot-path cost to
   * the pre-slice-116 globalThis property read.
   */
   ...
   * invalidates — see slice-99 / slice-87 docs and `_installHelperRecomputeBridge`
   * doc in manager.ts L501-664). The hook is optional (null until manager.ts
   * registers it via queueMicrotask after compile.ts's
   * `installCompilePipelinePart` runs); when null the push proceeds without
   * the bridge install. Errors from the hook are caught and swallowed —
   * matches pre-slice-116 IIFE wrapper's try/catch behavior.
   *
   ...
   * class-based-helper instance creation block (when a fresh class-helper
   * is created and inserted into `_tagHelperInstanceCache` — slice-99
   * sibling cache). Direct module-local call bypasses the bridge (slice-22/
   * 24/27/30/.../99 intra-file precedent — zero overhead).
   *
   * Pre-slice-116 writer pattern (3 sites, all cross-file):
   ...
   *
   * The pre-slice-116 push-hook install path lived in a manager.ts IIFE
   * (manager.ts:672-706) that mutated the array's `.push` method in place
   * — intercepting every push with `_installHelperRecomputeBridge(item)`
   * BEFORE delegating to the original push. Slice-116 moves the hook
   * dispatch INTO the `pushHelperInstance` body itself, eliminating the
   * prototype mutation and giving the hook install a clean typed-bridge
   * surface (`setHelperInstancePushHook`). Net behavior is identical:
   * every push triggers the recompute bridge install (best-effort),
   * then the array receives the instance.
   *
   * Bridge-not-yet-installed semantics: optional-chain `pushHelperInstance?.()`
   * is a no-op before compile.ts's `installCompilePipelinePart` at EOF
   * fires. The pre-slice-116 cross-file pushes had the same effective
   * semantics — if compile.ts hadn't loaded (e.g., classic-Ember build),
   * the destruction array would be undefined and the `Array.isArray` guard
   * would short-circuit. The destroy-tracking is a GXT-mode-only concern;
   * skipping it in classic-Ember has no observable effect (the helper
   * instances are destroyed by Ember's classic owner.destroy() path).
   *
   * Fast-check: optional-chain call (1 indirection if hook installed) +
   * 1 array.push. Zero allocations on hot path.
   */
   ...
   *
   * Pre-slice-116 the hook install lived in a manager.ts module-init
   * IIFE (`_installHelperInstancesHook` at manager.ts:672-706) that
   * mutated the globalThis array's `.push` method in place — wrapping
   * every push with a try/catch that called
   * `_installHelperRecomputeBridge(item)` BEFORE delegating to the
   * original push. Slice-116 graduates this hook install to a typed
   * bridge surface: manager.ts calls
   * `compilePipeline.setHelperInstancePushHook?.(_installHelperRecomputeBridge)`
   * via queueMicrotask deferral (so the call fires AFTER compile.ts's
   * `installCompilePipelinePart` registers the setter). The
   * `pushHelperInstance` implementation in compile.ts fires the
   * registered hook in a try/catch that matches the pre-slice-116
   * IIFE wrapper's error swallowing — identical best-effort semantics.
   *
   ...
   *
   * Fast-check: one assignment to `_gxtHelperInstancePushHook`. Zero
   * allocations.
   */
   ...
   *
   * The two cross-package/cross-file readers at:
   *
   *   - `gxt-backend/manager.ts:3320` (`_buildRenderTree` outlet branch
   *      — walks `topOutletRef.outlets.main...` to emit the
   *      `{{outlet}} for X / X / {{outlet}} for Y / Y / ...`
   *      `renderTreeParts` sequence that matches Glimmer VM's render-
   *      tree output);
   *   - `glimmer/lib/renderer.ts:1133` (OutletView property-driven
   *      re-render fallback — uses the captured ref when
   *      `(gxtRoot as any).ref` is missing);
   *
   * route through `compilePipeline.getTopOutletRef?.()` (see below). The
   * pre-slice-111 globalThis slot returned `undefined` until the writer
   * fired at least once; the bridge fallback `?? undefined` preserves
   * the same not-yet-set semantics.
   *
   * Pre-slice-111 topology (audited verbatim against current source):
   *   Writers (1 site, in glimmer package):
   *     - glimmer/lib/templates/root.ts:956 — captures
   *       `instance.outletRef` after the outlet-render block, inside the
   *       closure-built re-render function.
   *   Readers (2 sites, cross-package + cross-file):
   *     - gxt-backend/manager.ts:3320 (`_buildRenderTree` outlet
   *       hierarchy walker — slice-110 sibling, gated on
   *       `__currentOutletState` + `isInOutletRender()` from slice 110);
   *     - glimmer/lib/renderer.ts:1133 (OutletView re-render fallback —
   *       reads the captured ref when `gxtRoot.ref` is missing).
   *
   * GXT-runtime ownership audit: 0 matches under
   * `node_modules/.pnpm/@lifeart+gxt` published artifacts, 0 in repo-root
   * `index.html`, 0 in scripts/, 0 in dist/, 0 in `packages/demo/`, 0 in
   * harness test runners. The slot is purely an Ember-internal outlet-
   * ref-capture marker — no external readers/writers to retain.
   *
   * Bridge shape decision: paired `setTopOutletRef(ref): void` (writer)
   * + `getTopOutletRef(): any` (reader). The slot carries a captured-ref
   * VALUE (not a flag toggle around a body), so the natural shape is
   * set/get rather than the slice-110 `withX(fn)` scope-modifier wrap.
   * Get/set paired surface mirrors the pre-slice-111 globalThis assign-
   * and-read pattern directly. Two new bridge methods, no new install-
   * API namespace (registered inside the existing
   * `installCompilePipelinePart` block at compile.ts EOF alongside the
   * slice-110 `withInOutletRender` / `isInOutletRender` paired entries).
   *
   * Namespace decision: `compilePipeline`. The captured ref is
   * semantically a render-pipeline state-cluster member alongside
   * slice-110's `_gxtInOutletRenderFlag` — both are written by root.ts's
   * outlet-render path and consumed by manager.ts's `_buildRenderTree`
   * outlet branch. Same namespace category as slice 110.
   *
   * State-home: `gxt-backend/compile.ts`. The canonical state is the
   * module-local `_gxtTopOutletRef` binding defined alongside slice-110's
   * `_gxtInOutletRenderFlag` (the paired outlet-render machinery).
   * Although the writer lives in `glimmer/lib/templates/root.ts` (cross-
   * package contributor — same as slice 110 / 96 / 108) and the readers
   * live in `gxt-backend/manager.ts` + `glimmer/lib/renderer.ts`, the
   * bridge state lives in compile.ts because:
   *   1) State-home convention is compile.ts for the compilePipeline
   *      ref/value-capture category (slice 65 / 100 precedent);
   *   2) The state belongs to the same outlet-render state cluster as
   *      slice-110's `_gxtInOutletRenderFlag` — paired sibling slots
   *      both written by root.ts's outlet-render path;
   *   3) Installing it from `installCompilePipelinePart` at compile.ts
   *      EOF mirrors slice-110 — no cycle introduced.
   *
   * Bridge-not-yet-installed edge: the cross-package writer in
   * `glimmer/lib/templates/root.ts` uses
   * `getGxtRenderer()?.compilePipeline.setTopOutletRef?.(instance.outletRef)`.
   * The optional chain short-circuits to `undefined` when either the
   * renderer or the method is not yet installed; the captured ref is
   * silently dropped on the floor — matching the pre-slice-111 semantics
   * where, if no read site fired before the next write, the ref had no
   * downstream effect anyway. The cross-package/cross-file readers use
   * `getGxtRenderer()?.compilePipeline.getTopOutletRef?.() ?? undefined`
   * — defaulting to `undefined` matches the pre-slice-111 globalThis
   * read which returned `undefined` when the slot had not yet been set.
   ...
   * any template renders), and root.ts only writes the ref during the
   * first outlet render — well after compile.ts has finished its install
   * pass. For classic-Ember builds (no GXT backend loaded), root.ts's
   * outlet-render branch does not execute the gxt-templated path, so
   * neither writer nor readers fire and the bridge short-circuit is moot.
   *
   * Re-entrancy / overwrite contract: each write OVERWRITES the prior
   * captured ref. There is no save-restore wrap (that would be the
   * slice-110 `withInOutletRender` shape) because the captured ref is a
   * persistent latest-value, not a scoped frame. Successive outlet
   * renders capture progressively-newer refs, and consumers always read
   * the LAST-CAPTURED ref. In practice each top-level outlet render
   * produces a fresh `instance.outletRef`, and the `_buildRenderTree`
   * reader fires after the writer has captured the current frame's ref.
   *
   * Hot-path concern: NONE. The writer fires ONCE per top-level outlet
   * render. The manager.ts reader fires once per `_buildRenderTree` call
   * (DevTools render-tree inspection — rare). The renderer.ts reader
   * fires once per OutletView property-driven re-render. The bridge
   * dispatch overhead vs. globalThis read/write is negligible at this
   * call frequency.
   *
   * Fast-check: the implementations are trivial — `function
   * _gxtSetTopOutletRef(ref) { _gxtTopOutletRef = ref; }` and `function
   * _gxtGetTopOutletRef() { return _gxtTopOutletRef; }`. Mirrors the
   * slice-65 / 100 ref-capture get-only family but adds a paired writer
   * because the writer is cross-package (root.ts → compile.ts state).
   *
   * After slice-111 the `__gxtTopOutletRef` globalThis slot is DROPPED.
   * The canonical state is `_gxtTopOutletRef` in compile.ts. Net
   * globalThis surface delta: -1 slot. +2 paired bridge methods on
   * `compilePipeline`.
   *
   ...
   * above. Returns `undefined` if no outlet render has yet captured a
   * ref (matching the pre-slice-111 `(globalThis as any).__gxtTopOutletRef`
   * read which returned `undefined` for an unset slot).
   *
   * Slice-111 (Cluster B): paired with `setTopOutletRef` (above). The
   * two cross-package/cross-file readers at `gxt-backend/manager.ts:3320`
   * (`_buildRenderTree` outlet hierarchy walker) and
   * `glimmer/lib/renderer.ts:1133` (OutletView re-render fallback) route
   * through `compilePipeline.getTopOutletRef?.() ?? undefined`. The
   * `?? undefined` fallback preserves the pre-slice-111 globalThis-read
   * semantics. See `setTopOutletRef` doc above for the full pre/post
   * topology, state-home decision, and bridge-not-yet-installed edge.
   *
   * Mirrors the slice-65 / 100 ref-capture get-only family.
   *
   * Previously: `(globalThis as any).__gxtTopOutletRef`.
   */
  getTopOutletRef?(): any;
   ...
   *   if (wasRendering) (globalThis as any).__gxtSkipTextEffects = true;
   *   try {
   *     // outer initial render OR inner _doRender body — depending on site
   *     if (wasRendering) (globalThis as any).__gxtSkipTextEffects = true;
   *     ...render the template...
   *   } finally {
   ...
   *   }
   *
   * around each renderComponent template render so that nested
   * `renderComponent` calls (called during an existing render pass — e.g.
   * from a `@cached` getter during template evaluation) suppress GXT-effect
   * creation for text nodes. The single cross-package reader at
   * `gxt-backend/compile.ts:15624` (inside the `$_text` reactive-binding
   * setup of the glimmer-template forked branch) reads the slot and
   * short-circuits the `gxtEffect(() => { ... })` setup if truthy —
   * returning the bare text node directly instead.
   *
   ...
   * tracked deps change. Independent text effects in the nested render
   * would fire out-of-order (before the parent re-renders) and cause
   * double getter evaluations. Top-level renderComponent calls
   * (wasRendering=false) DO need text effects for trackedObject
   * reactivity, hence the conditional set-true gated on `wasRendering`.
   *
   * Pre-slice-115 topology (audited verbatim against current source):
   *
   *   Writers (5 sites at 7 lines, all in `glimmer/lib/renderer.ts`'s
   *   `_renderComponent` entrypoint):
   *     - L2484 (save prev) + L2486 (conditional set TRUE if `wasRendering`):
   *       outer initial-render frame entry — save current flag, set TRUE
   *       only when called nested inside an existing render pass.
   *     - L2508 (save prev) + L2510 (conditional set TRUE if
   *       `wasRenderingLocal`) + L2525 (restore prev in `finally`): inner
   *       `_doRender` classic-reactor body — independent save-restore pair
   *       around the scoped re-render scope (classic-reactor fires this
   *       body on classic-tag dirty after initial render completes).
   *     - L2612 (re-assert set TRUE inside outer try-block when
   *       `wasRendering`) + L2622 (restore prev in outer `finally`):
   *       outer initial-render frame `try / finally` — the re-assert is a
   *       defensive reinforcement (the L2486 write is already true if
   *       wasRendering, but the inner classic-reactor's L2525 finally may
   *       have restored to a different prev value if it fired BEFORE
   *       initial render completed; the L2612 re-assert ensures the
   *       initial-render template body sees the correct flag).
   *
   *   Reader (1 cross-package site):
   *     - gxt-backend/compile.ts:15624 — inside the `$_text` reactive-
   *       binding fork of the glimmer-template result handling. Tests the
   *       slot via `if ((globalThis as any).__gxtSkipTextEffects) { return
   *       textNode; }` — returns the bare text node without setting up a
   *       `gxtEffect` if the flag is truthy.
   *
   * GXT-runtime ownership audit: 0 matches under
   * `node_modules/.pnpm/@lifeart+gxt` published artifacts, 0 in repo-root
   * `index.html`, 0 in scripts/, 0 in `packages/demo/`, 0 in harness test
   * runners. The slot is purely an Ember-internal nested-renderComponent
   * gate — no external readers/writers to retain.
   *
   * Bridge shape decision: paired `setSkipTextEffects(value: boolean):
   * void` (writer) + `getSkipTextEffects(): boolean` (reader) methods on
   * `GxtCompilePipelineCapabilities`. Set/get paired surface mirrors the
   * pre-slice-115 globalThis assign-and-read pattern directly. The
   * pre-slice-115 writer-pair shape is NOT a balanced wrap-the-body-in-
   * begin-end (the `wasRendering` conditional only fires set-true for
   * the truthy branch — the falsy branch leaves the prior value
   * untouched). That conditional save/conditional-set/unconditional-restore
   * pattern is best expressed as paired set/get with the writers retaining
   * the conditional logic, not as a `withSkipTextEffects(fn)` wrap.
   * Mirrors slice-111's `setTopOutletRef` / `getTopOutletRef` (paired
   * set/get for a captured VALUE) — but boolean-typed here rather than
   * ref-typed. Two new bridge methods, no new install-API namespace
   * (registered inside the existing `installCompilePipelinePart({ ... })`
   * block at compile.ts EOF alongside slices 110/111/112/113 entries).
   *
   * Namespace decision: `compilePipeline`. The flag is semantically part
   * of the GXT render pipeline — a per-frame gate on the text-node
   * reactive-effect setup inside the glimmer-template `$_text` fork.
   * Same namespace category as slice-110's `withInOutletRender`, slice
   * 111's `setTopOutletRef`, slice 112's `withForceRerender`, and slice
   * 113's `setRootOutletRerender` — all live on `compilePipeline`.
   *
   * State-home: `gxt-backend/compile.ts`. Same precedent as slices
   * 110/111/112 — bridge state for compilePipeline-namespace ref/flag
   * captures lives in compile.ts, even when the writer is cross-package
   * (in `glimmer/lib/renderer.ts`) and the reader is intra-file. The
   * sole reader at compile.ts L15624 reads the canonical module-local
   * directly (intra-file zero-overhead, slice-30 precedent) rather than
   * round-tripping through the bridge predicate.
   *
   * Bridge-not-yet-installed edge: the five cross-package writer sites
   * in `glimmer/lib/renderer.ts` use `getGxtRenderer()?.compilePipeline.
   * setSkipTextEffects?.(value)`. The optional chain short-circuits to
   * `undefined` when either the renderer or the method is not yet
   * installed; in that case the write is silently dropped (matching the
   * pre-slice-115 semantics where the globalThis slot would simply not
   * affect the cross-package reader, which would then default to falsy).
   * The reader uses the intra-file module-local directly, so its
   * fallback is the initial `false` value of the module-local binding —
   * matching the pre-slice-115 `(globalThis as any).__gxtSkipTextEffects`
   * read which returned `undefined` (falsy) when the slot had not yet
   * been written. In practice the bridge IS installed by the time
   * either site fires: `installCompilePipelinePart` runs at compile.ts
   * module init (before any template renders), and renderer.ts's
   * `_renderComponent` only writes the flag during a render — well after
   * compile.ts has finished its install pass. For classic-Ember builds
   * (no GXT backend loaded), renderer.ts's `_renderComponent` entrypoint
   * also short-circuits the writes (and there is no compile.ts reader
   * to consult), so the bridge short-circuit is moot.
   *
   * Save-restore semantics: the pre-slice-115 writer pairs preserve a
   * "save prior on entry, restore prior on finally" pattern via
   * `prevSkipTextEffects = read; ... try { ... } finally { write(prev); }`.
   * The slice-115 migration preserves this verbatim — the renderer.ts
   * writers still read the prior value via `getSkipTextEffects()` on
   * entry, store it in a local, and write it back via
   * `setSkipTextEffects(prev)` in `finally`. Net behavior identical.
   *
   * Hot-path concern: NONE. The writer fires once per renderComponent
   * call (top-level public API — typically zero to a handful per QUnit
   * test). The reader fires once per text-node binding inside a
   * renderComponent template render. The bridge dispatch overhead vs.
   * globalThis read/write is negligible at this call frequency.
   *
   * Fast-check: the implementations are trivially cheap —
   * `function _gxtSetSkipTextEffects(v) { _gxtSkipTextEffectsFlag = v; }`
   * and `function _gxtGetSkipTextEffects() { return
   * _gxtSkipTextEffectsFlag; }`. Same body shape as slices 111
   * (setTopOutletRef/getTopOutletRef).
   *
   * After slice-115 the `__gxtSkipTextEffects` globalThis slot is
   * DROPPED. The canonical state is `_gxtSkipTextEffectsFlag` in
   * compile.ts. Net globalThis surface delta: -1 slot. +2 paired bridge
   * methods on `compilePipeline`.
   *
   * Previously: `(globalThis as any).__gxtSkipTextEffects = value`.
   */
   ...
   *
   * Slice-115 (Cluster B): paired with `setSkipTextEffects` (above).
   * The sole cross-package reader at `gxt-backend/compile.ts:15624`
   * (inside the `$_text` reactive-binding fork of the glimmer-template
   * result handling) is intra-file with the state-home, so it reads
   * the module-local `_gxtSkipTextEffectsFlag` binding DIRECTLY rather
   * than round-tripping through this bridge predicate (slice-30
   * precedent — intra-file zero-overhead). This bridge accessor is
   * provided for symmetry with `setSkipTextEffects` and for any future
   * external readers that may need to consult the flag.
   *
   * Matches slice-111's `getTopOutletRef()` body shape (paired
   * accessor for a captured VALUE).
   *
