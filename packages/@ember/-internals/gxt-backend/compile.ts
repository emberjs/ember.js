...
(globalThis as any).__gxtExternalSchedule = function () {
  ...
  // (module-local in this file) or via
  // `compilePipeline.setPendingSyncFromPropertyChange?.(true)` for cross-
  // package writers (templates/root.ts outlet rerender + routing/router.ts
  // transition LinkTo path).
};
...
// Slice-35 (Cluster B): graduates the `__gxtHadPendingSync` boolean flag from
// the pre-slice-35 `globalThis.__gxtHadPendingSync` slot to the module-local
// boolean `_gxtHadPendingSyncFlag` in `compile.ts`, paired with a 2-method
// get/set bridge surface (`getHadPendingSync` + `setHadPendingSync`) on the
// `compilePipeline` namespace. The flag survives `__gxtSyncDomNow`'s Phase-1
// gxtSyncDom call but is cleared by `__gxtForceEmberRerender`'s finally-block
// (its companion slot `__gxtSyncIsPropertyDriven`, graduated in slice 34, was
// introduced precisely because this flag is cleared mid-flush). The pre-slice-
// 35 topology was 5 writers (4 intra-gxt-backend + 1 cross-package in
// `glimmer/lib/renderer.ts:1373`) and 4 readers (2 intra-gxt-backend +
// 2 in `glimmer/lib/renderer.ts:989` and `:1292`). Slice 35 routes intra-
// `compile.ts` writers/readers through `_gxtSetHadPendingSync` /
// `_gxtGetHadPendingSync` directly (slice-22/24/27/30/31/32/33/34 intra-file-
// writer precedent); the cross-file writer at `manager.ts:579` (helper
// recompute) routes through `compilePipeline.setHadPendingSync(true)`; the
// cross-package writer at `glimmer/lib/renderer.ts:1373` (force-rerender
// finally) routes through `compilePipeline.setHadPendingSync(false)`; the
// cross-package readers at `glimmer/lib/renderer.ts:989/1292` route through
// `compilePipeline.getHadPendingSync?.() ?? false`. Net globalThis surface
// delta: -1 slot. Bridge shape decision: paired get/set (slice-14 paired-
// methods pattern) instead of slice-20/22/23/24's read-only predicate
// because slice 35 has cross-file/cross-package WRITERS in addition to
// readers — both surfaces must be reachable.
...
// `__gxtCurrentHelperScope` ref-capture slot. Pre-slice-117 three intra-file
// save-restore writer pairs in compile.ts's `patchGlobalIf`/`wrapBranch`
// (at compile.ts:5488/5624, 5489 and 5592/5616, 5593) wrote
// `(globalThis as any).__gxtCurrentHelperScope = scope / prev` around the
// branch evaluation body (`fn.apply(this, branchArgs)` / `inner.apply(this,
// innerArgs)`), publishing a per-branch `Set<any>` (`trueBranchHelpers` /
// `falseBranchHelpers`) so cross-file class-helper instantiation paths could
// associate freshly-created helper instances with the enclosing `{{#if}}`
// branch's teardown scope (matching Ember's classic Helper lifecycle —
// destroy + willDestroy fire on branch swap, not only on component teardown).
// Three readers consulted the slot:
//
//   - compile.ts:10345 — intra-file read inside `$_tag` class-based-helper
//     instance creation block. After pushing the freshly-created instance
//     into `_tagHelperInstanceCache` + the destroy-tracking array, the
//     reader calls `ifScopeTag.add(helperInstance)` to wire the instance
//     to the enclosing branch scope.
//   - ember-gxt-wrappers.ts:918 — cross-file read inside `$_maybeHelper`'s
//     branch-swap recreation branch (cached bucket needs full recreation),
//     calls `ifScope2.add(destroyable)` after the bridge-pushed instance.
//   - ember-gxt-wrappers.ts:1092 — cross-file read inside `$_maybeHelper`'s
//     class-based-helper path (factory.create() cache miss), calls
//     `ifScope.add(instance)` after the bridge-pushed instance.
//
...
//
// State-home: `compile.ts` (this file). The canonical state is the
// module-local `_gxtCurrentHelperScope` binding declared below. State-home
// follows slice-111's `_gxtTopOutletRef` precedent (paired ref-capture slot
// in the compilePipeline ref/value-capture category — slice 65 / 100 / 111
// precedent for ref-capture state living in compile.ts). The writers ARE
// all intra-file (compile.ts's `patchGlobalIf`), so the bridge `set` surface
// is currently unused cross-file; we ship it anyway to mirror slice-111's
// paired set/get shape and to permit any future cross-file writer to
// extend through the typed surface rather than re-publishing on globalThis.
// Intra-file writers and the intra-file reader at compile.ts:10345 use the
// module-local `_gxtCurrentHelperScope` directly (slice-22/24/27/.../99/116
// intra-file precedent — zero overhead, bypasses bridge dispatch).
//
// Paired bridge shape: `setCurrentHelperScope(scope: any): void` (writer)
// + `getCurrentHelperScope(): any` (reader). Get/set paired surface mirrors
// the pre-slice-117 globalThis assign-and-read pattern directly. The slot
// carries a captured-scope VALUE (a `Set<any>` or null/undefined; intra-file
// frames save/restore the prior value around a body), not a flag toggle —
// natural shape is set/get rather than a `withX` scope-modifier wrap.
// Two new bridge methods, no new install-API namespace (registered inside
// the existing `installCompilePipelinePart` block at compile.ts EOF
// alongside the slice-111 `setTopOutletRef` / `getTopOutletRef` paired
// entries).
//
// Re-entrancy / overwrite contract: each write OVERWRITES the prior captured
// scope. Intra-file callers in `patchGlobalIf`/`wrapBranch` save the prior
// scope into a local `prev`/`prev2` binding and restore it in `finally` so
// nested branch evaluations correctly restore the outer scope. Bridge
// writers are responsible for save-restore themselves (the bridge does NOT
// auto-wrap — same as slice-111's `setTopOutletRef`).
//
// Net globalThis surface delta: -1 slot (`__gxtCurrentHelperScope`). The
// three intra-file writer pairs and the intra-file reader graduate to the
// module-local binding directly; the two cross-file readers in
// ember-gxt-wrappers.ts route through
// `getGxtRenderer()?.compilePipeline.getCurrentHelperScope?.()`. The
// optional-chain fallback at the readers matches the pre-slice-117
// globalThis read which returned `undefined` when the slot had not yet
// been set.
...
// `(globalThis as any).__gxtIsForceRerender = true/false` around the
// `classicRoot.render()` body, and eight cross-package + cross-file
// readers consulted the slot:
//
//   - `gxt-backend/manager.ts:815` (`_shouldWarnStyle`);
//   - `gxt-backend/manager.ts:1557` (pool-claim sweep on factory swap);
//   - `gxt-backend/manager.ts:2934` (`pushedUpdatedInstance` skip);
//   - `gxt-backend/manager.ts:5996` (ember-component init bypass);
//   - `gxt-backend/manager.ts:11903` (`_buildDom` reused-from-pool gate);
//   - `gxt-backend/compile.ts:10832` (`__forceRerenderSnapshot` in $_tag);
//   - `gxt-backend/compile.ts:11425` (style-binding XSS warn suppression);
//   - `gxt-backend/outlet.gts:290` (engine-instance cache reuse);
//   - `glimmer/lib/templates/root.ts:588` (OutletView short-circuit);
//
// Slice-112 graduates BOTH sides to typed bridge methods. The canonical
// state is the module-local `_gxtIsForceRerenderFlag` boolean below;
// the globalThis slot is DROPPED.
//
// State-home: `compile.ts` (this file). Same state-home as the other
// compilePipeline boolean toggles (slice 17/18/22/23/24/27/30/31/97/108/
// 110 precedent — the canonical "scope-modifier boolean on the GXT
// render/sync pipeline" category). Although the pre-slice-112 writer
// lives in `glimmer/lib/renderer.ts` (cross-package contributor — same
// pattern as slices 96/108/110/111) and most readers live in
// `gxt-backend/`, the bridge state lives here because compile.ts is the
// canonical home for the compilePipeline boolean cluster.
//
// Paired bridge shape: `withForceRerender<T>(fn): T` (writer) +
// `isForceRerender(): boolean` (reader). Mirrors slice-18 / slice-22 /
// slice-108 / slice-110 paired surfaces. Pure balanced save-restore
// wrap (no depth counter, no transition side-effects) — same shape as
// slices 18/22/108/110. Re-entrancy contract: save the prior flag on
// entry, set to `true`, restore prior on `finally` exit. In practice
// no nested force-rerender occurs (the body is the top-level
// `classicRoot.render()` frame and is not re-entered during its body),
// but the save-restore is the canonical Cluster B shape.
//
...
// migration preserves that catch — the `fn` argument passed to
...
//
// Net globalThis surface delta: -1 slot (`__gxtIsForceRerender`).
// The pre-slice-112 dual writer at `glimmer/lib/renderer.ts:1507/1516`
// routes through `compilePipeline.withForceRerender?.(fn) ?? fn()`, and
// the eight cross-package + cross-file readers route through
// `compilePipeline.isForceRerender?.() ?? false`. The optional-chain
// fallback at the readers matches the pre-slice-112 `if (undefined)`
// falsy coercion which treated `undefined` as "not in force-rerender".
...
// Slice-89 (Cluster B): graduates the `__gxtDestroyReattachInProgress` boolean
// flag from the pre-slice-89 `globalThis.__gxtDestroyReattachInProgress` slot
// to the module-local boolean `_gxtDestroyReattachInProgressFlag` in
// `compile.ts`, paired with a 2-method get/set bridge surface
// (`getDestroyReattachInProgress` + `setDestroyReattachInProgress`) on the
// `compilePipeline` namespace. The flag is set TRUE for the duration of a
// destroy-phase reattach loop that temporarily re-attaches disconnected
// component elements to `document.body` / `#qunit-fixture` so that
// `willDestroyElement` / `willClearRender` hooks see
...
// elements are detached again. The flag gates the `<ember-outlet>` custom
// element's `connectedCallback` body (`glimmer/lib/templates/outlet.ts:30`):
// when a destroy-phase reattach fires `connectedCallback` on an inner
// `<ember-outlet>` (e.g., one nested inside a just-removed component wrapper
// like `root-9`), the callback MUST skip rendering — without this gate the
// callback would read `__currentOutletState` (the NEW route after the
// transition), render the new route's template, and corrupt the parentView
// stack (the new route's components would get `parentView = root-9` and
// disappear from `getRootViews`).
//
// The pre-slice-89 topology was 4 writers (2 in `manager.ts` cross-file pair
// at L4852/L4866 set TRUE/FALSE around the `__gxtDestroyUnclaimedPoolEntries`
// Phase 1 reattach IIFE + 2 in `compile.ts` intra-file pair at L5827/L5841
// set TRUE/FALSE around the outlet-tracking-arg inverse-branch destroy
// reattach IIFE) and 1 reader (cross-package in
// `glimmer/lib/templates/outlet.ts:30` `connectedCallback` guard). Slice 89
// routes the 2 intra-`compile.ts` writers through the module-local
// `_gxtSetDestroyReattachInProgress` helper directly (slice-22/24/27/30/31/
// 32/33/34/35/36/37/38/40/41 intra-file precedent); the 2 cross-file writers
// in `manager.ts` and the 1 cross-package reader in
// `glimmer/lib/templates/outlet.ts` route through the new paired bridge
// methods. Net globalThis surface delta: -1 slot. Bridge shape decision:
// paired get/set (slice-14/35/36/37/38/40/41 paired-methods pattern, same as
// slice 41) because slice 89 has cross-file WRITERS (`manager.ts`) and a
// cross-package READER (`glimmer/lib/templates/outlet.ts`) — both surfaces
// must be reachable via the bridge. ZERO new import edges: `manager.ts`
// already imports `getGxtRenderer` (slice 6+) and `outlet.ts` already imports
// `getGxtRenderer` (slice 3).
//
// State home: `compile.ts` (alongside the other intra-file paired bridges
// added in slices 37/38/40/41) per the "canonical state lives where it's
// primarily mutated" rule (2 of 4 writers are intra-file in compile.ts) and
// consistency with the other module-local boolean-flag paired bridges.
...
// `compile.ts`, paired with a 2-method get/set bridge surface
// (`getPendingSyncFromPropertyChange` + `setPendingSyncFromPropertyChange`)
// on the `compilePipeline` namespace. Flag 3 of the 4-flag pending-sync
// cluster (`__gxtPendingSync` / `__gxtHadPendingSync` /
// `__gxtPendingSyncFromPropertyChange` / `__gxtSyncIsPropertyDriven`); flag
// 4 closed in slice 34 and flag 2 closed in slice 35. The flag is set TRUE
// when `__gxtTriggerReRender` observes a real property change (so
// `__gxtSyncDomNow` can distinguish a property-driven sync from a cell-
// creation-during-initial-render sync) and cleared at well-known
// boundaries: at the start of `__gxtSyncDomNow` (after capturing into
// `__gxtHadPendingSync` and `__gxtSyncIsPropertyDriven`), at the end of
// every `runTask` / `runAppend` (test-helper writer-contract — flush the
// flag so the setInterval(16ms) fallback does not pick up stale sync
// state for the next test), in the cross-test teardown phases (additional
// belt-and-suspenders), in `__gxtPostRenderHooks` save/restore around
// `didUpdate`/`didRender` (to detect whether the hook itself produced
// property changes), and in `wrapHandler`'s tail finally (so user-
// interaction handler property changes do not survive past the handler).
// The pre-slice-36 topology was 14 writers (5 intra-`compile.ts` +
// 4 in `gxt-backend/manager.ts` + 1 cross-package in
// `glimmer/lib/templates/root.ts:1075` outlet rerender + 1 cross-package
// in `routing/router.ts:106` transition LinkTo + 4 in `internal-test-
// helpers` — 3 in `run.ts` and 1 each in `test-cases/{rendering,
// abstract, abstract-application}.ts`) and 2 readers (both intra-
// `compile.ts` at L5671 and L5683 inside `__gxtSyncDomNow`, where the
// flag is captured into `_gxtSetHadPendingSync` and
// `_gxtSetSyncIsPropertyDriven` before being cleared). Slice 36 routes
// intra-`compile.ts` writers/readers through the module-local
// `_gxtSetPendingSyncFromPropertyChange` / `_gxtGetPendingSyncFromPropertyChange`
// helpers directly (slice-22/24/27/30/31/32/33/34/35 intra-file
// writer/reader precedent); the 4 cross-file writers in `manager.ts` (4328
// save-read, 4330 clear, 4352 restore, 4356 restore-or, 11019 handler
// clear) route through the new bridge methods. The 2 cross-package writers
// (templates/root.ts:1075 outlet + routing/router.ts:106 transition) route
// through `compilePipeline.setPendingSyncFromPropertyChange?.(true)`. The
// 4 test-helper writers (contract writers — clear state in teardown /
// runTask / runAppend / render postprocess) route through the bridge
// setter, establishing the test-helper-bridge-writer pattern that flag 1
// (`__gxtPendingSync`) will reuse in slice 37. Net globalThis surface
// delta: -1 slot. Bridge shape decision: paired get/set (slice-14 paired-
// methods pattern, same as slice 35) instead of slice-20/22/23/24's read-
// only predicate because slice 36 has cross-file/cross-package WRITERS
// across 6 packages — both surfaces must be reachable.
  ...
  g.$_if = function patchedIf(conditionOrCell: any, trueBranch: any, falseBranch: any, ctx: any) {
    ...
    const wrapBranch = (fn: any, scope: Set<any>) => {
      ...
      return function wrappedBranch(this: any, ...branchArgs: any[]) {
        ...
        // Slice-117 (Cluster B): intra-file save-restore frame on the
        // module-local `_gxtCurrentHelperScope`. See doc above.
          ...
          if (isSyncing && !alreadyFired && capturedParent) {
            ...
            if (allPools) {
              ...
              if (candidates.length > 0) {
                ...
                // Slice-89 (Cluster B): write to module-local
                // `_gxtDestroyReattachInProgressFlag` via the module-private
                // setter (canonical state graduated from
                // `globalThis.__gxtDestroyReattachInProgress` in slice 89).
                // Intra-file writer routes through the module-local helper
                // directly per the slice-22/24/27/30/.../41 precedent. See
                // `getDestroyReattachInProgress` / `setDestroyReattachInProgress`
                // doc in gxt-bridge.ts and module-local definition above.
...
function _gxtSyncDomNow(): void {
  ...
  if (_gxtGetPendingSync()) {
    ...
    // (graduated from `globalThis.__gxtHadPendingSync`). See module-local
    // definition above for the migration narrative.
    // Slice-36 (Cluster B): read module-local
    // `_gxtPendingSyncFromPropertyChangeFlag` (graduated from
    // `globalThis.__gxtPendingSyncFromPropertyChange`). Intra-file reader
    // routes through the helper directly (no bridge indirection).
    ...
    // Slice-36 (Cluster B): clear module-local
    // `_gxtPendingSyncFromPropertyChangeFlag` after capturing it into the
    // two survivor flags above.
    ...
    try {
      ...
      // Slice-35 (Cluster B): read module-local `_gxtHadPendingSyncFlag`
      ...
      try {
        ...
        if (typeof syncAll === 'function') {
          ...
          if (
            ...
            // Slice-35 (Cluster B): write to module-local
            // `_gxtHadPendingSyncFlag` (graduated from
            // `globalThis.__gxtHadPendingSync`).
      ...
      if (mightBeComponent && managers?.component?.canHandle) {
              ...
              if (isClassBased) {
                ...
                if (
                  ...
                  try {
                    ...
                    // lifecycle test). Slice-117 (Cluster B): direct intra-file
                    // read of the module-local `_gxtCurrentHelperScope`.
        ...
        if (managers.component.canHandle(kebabName)) {
          ...
          const renderComponent = function __componentThunk() {
            ...
            // (zero overhead, mirrors slice-30 intra-file precedent for the
            // module-local sync-cycle counter). The snapshot captures the
            // flag BEFORE `handle()` runs so the post-`finally` block can
            // gate willRender/didUpdateAttrs fan-out on whether we were in
            // the force-rerender frame.
...
installCompilePipelinePart({
  ...
  // Slice-35 (Cluster B): graduates `__gxtHadPendingSync` from the globalThis
  // slot to a 2-method paired get/set bridge surface (slice-14 paired-methods
  // pattern). The flag is written by 5 sites pre-slice-35 (3 intra-`compile.ts`
  // + 1 in `gxt-backend/manager.ts:579` helper recompute + 1 cross-package
  // in `glimmer/lib/renderer.ts:1373` force-rerender finally) and read by 4
  // sites (2 intra-`compile.ts` + 2 cross-package in `glimmer/lib/renderer.ts`
  // L989 modifier-replay gate + L1292 force-rerender start capture). Intra-
  // file writers/readers route through the module-local `_gxtSetHadPendingSync`
  // / `_gxtGetHadPendingSync` helpers directly; cross-file/cross-package
  // writers route through `compilePipeline.setHadPendingSync(value)`;
  // cross-package readers route through `compilePipeline.getHadPendingSync?.()
  // ?? false`. Net -1 globalThis slot. See `getHadPendingSync` /
  // `setHadPendingSync` doc in gxt-bridge.ts and module-local definition
  // above (`_gxtHadPendingSyncFlag`).
  getHadPendingSync: _gxtGetHadPendingSync,
  setHadPendingSync: _gxtSetHadPendingSync,
  // Slice-36 (Cluster B): graduates `__gxtPendingSyncFromPropertyChange`
  // from the globalThis slot to a 2-method paired get/set bridge surface
  // (slice-14/35 paired-methods pattern). The flag is written by 14 sites
  // pre-slice-36 (5 intra-`compile.ts` + 4 in `gxt-backend/manager.ts` +
  // 1 cross-package in `glimmer/lib/templates/root.ts:1075` outlet
  // rerender + 1 cross-package in `routing/router.ts:106` transition
  // LinkTo + 4 in `internal-test-helpers` test-helper writer-contract
  // sites) and read by 2 sites (both intra-`compile.ts` inside
  // `__gxtSyncDomNow` at L5671 / L5683 where the flag is captured into
  // `_gxtSetHadPendingSync` and `_gxtSetSyncIsPropertyDriven` before being
  // cleared). Intra-file writers/readers route through the module-local
  // `_gxtSetPendingSyncFromPropertyChange` / `_gxtGetPendingSyncFromPropertyChange`
  // helpers directly; cross-file/cross-package writers (manager.ts,
  // glimmer, routing, internal-test-helpers) route through
  // `compilePipeline.setPendingSyncFromPropertyChange(value)`. Net -1
  // globalThis slot. See `getPendingSyncFromPropertyChange` /
  // `setPendingSyncFromPropertyChange` doc in gxt-bridge.ts and module-
  ...
  // Slice-89 (Cluster B): graduates `__gxtDestroyReattachInProgress` from the
  // globalThis slot to a 2-method paired get/set bridge surface (slice-14/35/
  // 36/37/38/40/41 paired-methods pattern). The flag is written by 4 sites
  // pre-slice-89 (2 in `manager.ts:4852/4866` cross-file pair around
  // `__gxtDestroyUnclaimedPoolEntries` Phase 1 reattach IIFE + 2 in
  // `compile.ts:5827/5841` intra-file pair around the outlet-tracking-arg
  // inverse-branch destroy reattach IIFE) and read by 1 cross-package site
  // pre-slice-89 (`glimmer/lib/templates/outlet.ts:30` `connectedCallback`
  // guard at the top of the body). Intra-file writers route through the
  // module-local `_gxtSetDestroyReattachInProgress` helper directly; cross-
  // file writers (manager.ts) route through
  // `compilePipeline.setDestroyReattachInProgress(value)`; cross-package
  // reader (outlet.ts) routes through
  // `compilePipeline.getDestroyReattachInProgress?.()`. Net -1 globalThis
  // slot. See `getDestroyReattachInProgress` / `setDestroyReattachInProgress`
  // doc in gxt-bridge.ts and module-local definition above
  ...
  // Slice-117 (Cluster B): paired set/get typed-bridge migration of the
  // `__gxtCurrentHelperScope` ref-capture slot. Pre-slice-117 three intra-
  // file save-restore writer pairs at compile.ts:5488/5624, 5489 and
  // 5592/5616, 5593 wrote `(globalThis as any).__gxtCurrentHelperScope =
  // scope / prev` around branch evaluation in `patchGlobalIf`/`wrapBranch`,
  // and three readers (1 intra-file at compile.ts:10345 inside `$_tag`
  // class-helper instance creation; 2 cross-file at ember-gxt-wrappers.ts
  // L918/L1092 inside `$_maybeHelper`'s branch-swap and class-based-helper
  // paths) read the slot to call `scope.add(instance)` and wire the helper
  // to the enclosing `{{#if}}` branch's teardown set. Slice-117 graduates
  // BOTH sides to typed bridge methods. The canonical state is the module-
  // local `_gxtCurrentHelperScope` defined above; the globalThis slot is
  // DROPPED. Net -1 globalThis surface, +2 paired bridge methods on
  // `compilePipeline`. See `setCurrentHelperScope` / `getCurrentHelperScope`
  // doc in gxt-bridge.ts. Mirrors slice-111's `setTopOutletRef` /
  // `getTopOutletRef` paired set/get shape directly (captured-value pair;
  // scope-typed here rather than ref-typed). Companion to slice-116's
  // helper-destroy-tracking array (the per-branch scope and the global
  // destroy-tracking array form a coherent state cluster — both written by
  // helper-instance creation paths and consumed by branch/component
  // teardown).
  setCurrentHelperScope: _gxtSetCurrentHelperScope,
  getCurrentHelperScope: _gxtGetCurrentHelperScope,
  // Slice-112 (Cluster B): paired typed-bridge migration of the
  // `__gxtIsForceRerender` boolean flag. Pre-slice-112 the writer pair at
  // `glimmer/lib/renderer.ts:1507/1516` (try-finally around
  // `classicRoot.render()`) wrote to `(globalThis as any).__gxtIsForceRerender
  // = true/false`, and eight cross-package + cross-file readers (5 in
  // manager.ts, 2 in compile.ts, 1 in outlet.gts, 1 in
  // glimmer/lib/templates/root.ts) consulted the slot with truthy
  // coercion to gate lifecycle/warning/destroy/pool-claim branches.
  // Slice-112 graduates BOTH sides to typed bridge methods. The
  // canonical state is the module-local `_gxtIsForceRerenderFlag`
  // defined above; the globalThis slot is DROPPED. Net -1 globalThis
  // surface, +2 paired bridge methods on `compilePipeline`. See
  // `withForceRerender` / `isForceRerender` doc in gxt-bridge.ts.
  // Mirrors slice-110's body shape directly (save-restore wrap, no
