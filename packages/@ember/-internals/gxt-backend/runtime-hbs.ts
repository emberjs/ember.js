...
// Bridge accessors for the ambient GXT root context.
 ...
 * compiler path (it treats `{{outlet}}` as a default slot yield), so the
 * source string is pre-processed here to mirror what the build-time plugin
  ...
  const templateFactory = {
    ...
    render(context: any, parentElement: Element | null) {
      ...
      try {
        ...
        // over the parent context when modules are deduplicated. The
        // canonical state is the module-local `_gxtRootContext` in compile.ts;
        // it is read/written through the `compilePipeline.getRootContext` /
