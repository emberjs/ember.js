...
// willDestroy/isDestroyed split. This module therefore keeps its own state and
// queue on top of GXT's destroyable so that:
//
//   * The Ember-visible API (registerDestructor/destroy/isDestroying/...) uses
//     this state machine and canonical scheduling paths.
//   * It still participates in GXT's destruction graph: when GXT destroys an
//     object as part of its own render-tree teardown, it bridges into this
...
// Intra-package import for the `compilePipeline.isSyncing` bridge predicate
// (the `__gxtSyncing` canonical state lives as a module-local boolean in
// `compile.ts`).
...
// (compat tests, application code) expect synchronous destruction. The active
// QUnit module name selects the correct path.
...
// `BrandedArray` distinguishes a destroyable that happens to be an
// `Array` from the internal "collection of destroyables" container. The
// upstream `@glimmer/destroyable` uses the same trick — without it,
// `iterate(['a'], fn)` walks the user-supplied array's elements instead
// of treating the array as a single destroyable, which makes the
// `Destroyables > parent can be an array` test crash with
// "Invalid value used as weak map key" when `getDestroyableMeta` is
// called with a string element (`'a'`).
const _BRANDED_ARRAY = Symbol('GxtBrandedArray');
type BrandedArray<T> = T[] & { [_BRANDED_ARRAY]: true };
type OneOrMany<T> = null | T | BrandedArray<T>;

function _isBrandedArray<T>(collection: OneOrMany<T>): collection is BrandedArray<T> {
  return Array.isArray(collection) && _BRANDED_ARRAY in collection;
}

function _brandArray<T>(arr: T[]): BrandedArray<T> {
  (arr as BrandedArray<T>)[_BRANDED_ARRAY] = true;
  return arr as BrandedArray<T>;
}
...
function push<T extends object>(collection: OneOrMany<T>, newItem: T): OneOrMany<T> {
  if (collection === null) {
    return newItem;
  } else if (_isBrandedArray(collection)) {
    collection.push(newItem);
    return collection;
  } else {
    return _brandArray([collection, newItem]);
  }
}

function iterate<T extends object>(collection: OneOrMany<T>, fn: (item: T) => void) {
  if (_isBrandedArray(collection)) {
...
function remove<T extends object>(collection: OneOrMany<T>, item: T, message: string | false) {
  ...
  if (_isBrandedArray(collection) && collection.length > 1) {
...
// graph traversal (e.g. when a parent template branch is torn down), the
// Ember-style destroy() runs so registered destructors fire via the runloop.
function ensureGxtBridge(_destroyable: Destroyable, _meta: DestroyableMeta<Destroyable>): void {
  // Bridging disabled: the state machine here handles the Ember-visible
  // semantics independently of GXT's internal graph.
  return;
}
...
export function associateDestroyableChild<T extends Destroyable>(parent: Destroyable, child: T): T {
  ...
  try {
    ...
    // Ignore; the relationship is still tracked here.
  }
...
// runtime), teardown can route through backburner.
...
export function destroy(destroyable: Destroyable): void {
  ...
  if (_getCurrentRunLoop() !== null || _deferredDestroyMode) {
    ...
    // The syncing flag is read through the bridge predicate (its canonical
    // state is a module-local `_gxtSyncingFlag` in `compile.ts`).
    _backburner.join(() => {
      _destroyDeferred(destroyable);
    });
...
function removeChildFromParent(child: Destroyable, parent: Destroyable) {
  ...
  // Mirror upstream `@glimmer/destroyable`: allow the
  // child to clear itself from the parent's children list while the parent
  // is in DESTROYING_STATE — the synchronous `iterate(children, destroy)`
  // has already finished by the time `scheduleDestroyed` callbacks run,
  // so this is safe and prevents the parent from retaining a stale
  // strong reference to the destroyed child (the
  // "parent no longer references child after cascaded destruction" test).
