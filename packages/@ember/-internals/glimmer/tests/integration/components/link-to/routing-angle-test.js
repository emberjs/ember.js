...
if (EMBER_GLIMMER_ANGLE_BRACKET_BUILT_INS) {
  ...
  moduleFor(
    ...
    class extends ApplicationTestCase {
      ...
      [`@test the <LinkTo /> component supports a custom disabledClass set via bound param`](
        assert
      ) {
        this.addTemplate(
          'index',
  ...
  moduleFor(
    ...
    class extends ApplicationTestCase {
      ['@test The <LinkTo /> component supports leaving off .index for nested routes'](assert) {
        ...
        this.addTemplate(
          'about.item',
          `<div id='item'><LinkTo @route='about'>About</LinkTo></div>`
        );
  ...
  moduleFor(
    ...
    class extends ApplicationTestCase {
      [`@test <LinkTo /> with null/undefined dynamic parameters are put in a loading state`](
        assert
      ) {
        assert.expect(19);
        let warningMessage =
