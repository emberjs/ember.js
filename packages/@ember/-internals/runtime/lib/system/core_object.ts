...
// Duplicate of version in property_get since we are unable to import it here for some reason.
interface HasUnknownProperty {
  unknownProperty: (keyName: string) => any;
}

function hasUnknownProperty(val: unknown): val is HasUnknownProperty {
  return (
    typeof val === 'object' &&
    val !== null &&
...
class CoreObject {
  ...
  */
  ...
  static extend(...mixins: Array<Mixin | Record<string, unknown>>) {
