...
moduleFor('PROPERTY_DID_CHANGE', class extends AbstractTestCase {
  ['@test alias and cp'](assert) {
    ...
    assert.ok(isWatching(obj, 'child.cost'), 'alias target `child.cost` is watched after consumption');

    assert.ok(!isWatching(obj, 'child.tax'), 'precond alias target `child.tax` is not watched');
    assert.equal(get(obj, 'tax'), undefined);
    // this is how PROPERTY_DID_CHANGE will get notified
    assert.ok(isWatching(obj, 'child.tax'), 'alias target `child.cost` is watched after consumption');
