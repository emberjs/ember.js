// Side-effect-only module: when imported, registers the per-opcode debug
// hooks (`debugBefore` / `debugAfter`) on `AppendOpcodes`. These hooks are
// only ever called when `LOCAL_DEBUG` is true (via the constant from
// `@glimmer/local-debug-flags`), so they're useless in production.
//
// Pulling them into their own module means production bundles that don't
// import this file drop `@glimmer/debug/lib/debug` (and its `OPERANDS`
// disassembler), `DebugLogger`, `VmSnapshot`, `opcodeMetadata`, etc. —
// 1–3 KB gzip of pure dev-only code.
