...
class CompileErrorTests extends RenderTest {
  ...
  @test "Unquoted attribute with expression throws an exception"() {
    this.assert.throws(() => this.env.preprocess('<img class=foo{{bar}}>'), expectedError(1));
    this.assert.throws(() => this.env.preprocess('<img class={{foo}}{{bar}}>'), expectedError(1));
    this.assert.throws(() => this.env.preprocess('<img \nclass={{foo}}bar>'), expectedError(2));
    this.assert.throws(() => this.env.preprocess('<div \nclass\n=\n{{foo}}&amp;bar ></div>'), expectedError(4));
