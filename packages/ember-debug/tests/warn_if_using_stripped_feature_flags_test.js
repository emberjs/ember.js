...
moduleFor('ember-debug - _warnIfUsingStrippedFeatureFlags', class extends TestCase {
  constructor() {
    super();
  ...
  ['@test Setting Ember.ENV.ENABLE_OPTIONAL_FEATURES truthy in non-canary, debug build causes a warning'](assert) {
    assert.expect(1);
  ...
  ['@test Enabling an unknown FEATURE flag in non-canary debug build does not cause a warning'](assert) {
    assert.expect(0);
