...
moduleFor(
  ...
  class extends AutobootApplicationTestCase {
        ...
        this.runTask(() => {
          app.boot().then(
            () => {
              assert.ok(
                false,
                'The boot promise should not resolve when there is a boot error'
              );
            },
            error => {
              assert.ok(
                error instanceof Error,
                'The boot promise should reject with an error'
              );
      ...
      MyApplication.initializer({
        ...
        initialize(App) {
          assert.ok(
            ...
            'initialize is passed an Application'
          );
        }
      });
    ...
    [`@test initializers can have multiple dependencies`](assert) {
      ...
      assert.ok(
        order.indexOf(b.name) < order.indexOf(afterB.name),
        'b < afterB'
      );
      assert.ok(
        order.indexOf(c.name) < order.indexOf(afterC.name),
        'c < afterC'
      );
    }

    [`@test initializers set on Application subclasses are not shared between apps`](
      assert
    ) {
      ...
      assert.equal(
        firstInitializerRunCount,
        1,
        'first initializer only was run'
      );
      assert.equal(
        secondInitializerRunCount,
        0,
      ...
      assert.equal(
        firstInitializerRunCount,
        1,
        'second initializer only was run'
      );
      assert.equal(
        secondInitializerRunCount,
        1,
      ...
      MyApplication.initializer({
        ...
        initialize() {
          assert.equal(
            this.myProperty,
            'cool',
            'should have access to its own context'
          );
        }
      });
